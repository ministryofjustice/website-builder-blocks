...
const drawerNavLevel1 = {
	selector: "header nav.wp-block-navigation.is-style-drawer > ul > li > .wp-block-navigation-item__content",
	handlerHorizontal: function (target, direction) {
		...
		const li = direction === "right" ? parent.nextElementSibling : parent.previousElementSibling;
	...
	handlerVertical: function (target, direction) {
		...
		if (isOpen && isDown) {
			...
			target.parentElement
				.querySelector(":scope > ul.wp-block-navigation-submenu > li > .wp-block-navigation-item__content")
				.focus();
		}
	},
...
const drawerNavLevel2 = {
	...
	handler: (target, direction) => {
		...
		const newIndex = positions.findIndex(l => l[0] === newPosition[0] && l[1] === newPosition[1]);

		// Did we find a newIndex to focus?
		if (newIndex !== -1 && menuItems[newIndex]) {
			menuItems[newIndex].querySelector(":scope > .wp-block-navigation-item__content").focus();
			return;
		}
		...
		if (direction === "up") {
			...
			parentList.parentElement.querySelector(".wp-block-navigation-submenu__toggle").focus();
		}
	},
};
 ...
 */
const detachedNav = {
	selector: "header nav.wp-block-navigation.is-style-detached .wp-block-navigation-item__content",
	handler: (target, direction) => {
		...
		const menuItems = Array.from(parentNav.querySelectorAll(".wp-block-navigation-item")).filter(el => {
