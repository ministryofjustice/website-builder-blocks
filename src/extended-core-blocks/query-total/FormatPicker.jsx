	...
	return (
		<VStack as="fieldset" spacing={4}>
			...
			<ToggleControl
				...
				help={`${__("Example:")}  ${__(sprintf(defaultFormatRange, 1, 10, 12))}`}
				checked={checked}
				onChange={checked =>
					onChange({
...
function NonDefaultControls({ rangeFormatSingle, rangeFormatMulti, onChange }) {
	const defaultOptions = [
		{
			...
			name: sprintf(__("Displaying %1$s – %2$s of %3$s", "wb_blocks"), "1", "10", "12"),
		},
		{
			key: 2,
			formatSingle: "Showing %1$s of %2$s results",
			formatRange: "Showing %1$s to %2$s of %3$s results",
			name: sprintf(__("Showing %1$s to %2$s of %3$s results", "wb_blocks"), "1", "10", "12"),
		},
	];
	...
	const [isCustom, setIsCustom] = useState(
		() =>
			!!rangeFormatSingle &&
			!!rangeFormatMulti &&
			!suggestedOptions.some(o => o.formatSingle === rangeFormatSingle && o.formatRange === rangeFormatMulti),
	);
				...
				value={
					isCustom
						...
						: (suggestedOptions.find(
								option => option.formatSingle === rangeFormatSingle && option.formatRange === rangeFormatMulti,
							) ?? customOption)
				}
			...
			{isCustom && (
				<>
					<TextareaControl
						...
						onChange={value => onChange({ rangeFormatSingle: value, rangeFormatMulti })}
						rows={2}
						__nextHasNoMarginBottom={true}
					/>
					<TextareaControl
						...
						onChange={value => onChange({ rangeFormatSingle, rangeFormatMulti: value })}
						rows={2}
						__nextHasNoMarginBottom={true}
					/>
				</>
