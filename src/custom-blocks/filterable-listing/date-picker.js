 ...
 * @property {string} [weekStartDay] - First day of the week in calendar view (monday or sunday only)
 */
...
function DatePicker($module, config = {}) {
	...
	this.buttonClass = "wb-datepicker__button";
	this.selectedDayButtonClass = "wb-datepicker__button--selected";
	this.currentDayButtonClass = "wb-datepicker__button--current";
	this.todayButtonClass = "wb-datepicker__button--today";

	this.$module = $module;
	this.$input = $module.querySelector(".wb-js-datepicker-input");
}
...
DatePicker.prototype.initControls = function () {
	...
	this.$calendarButton = this.$module.querySelector(".wb-js-datepicker-toggle");
	this.$dialogTitle = this.$dialog.querySelector(".wb-js-datepicker-month-year");

	this.createCalendar();

	this.$prevMonthButton = this.$dialog.querySelector(".wb-js-datepicker-prev-month");
	this.$prevYearButton = this.$dialog.querySelector(".wb-js-datepicker-prev-year");
	this.$nextMonthButton = this.$dialog.querySelector(".wb-js-datepicker-next-month");
	this.$nextYearButton = this.$dialog.querySelector(".wb-js-datepicker-next-year");
	this.$cancelButton = this.$dialog.querySelector(".wb-js-datepicker-cancel");
	this.$okayButton = this.$dialog.querySelector(".wb-js-datepicker-okay");
...
DatePicker.prototype.createDialog = function () {
	...
	$dialog.id = this.id;
	$dialog.setAttribute("class", "wb-datepicker__dialog");
...
DatePicker.prototype.createCalendar = function () {
	...
	for (let i = 0; i < 6; i++) {
		...
		for (let j = 0; j < 7; j++) {
			...
			// wp-element-button class brings in button focus styling
 ...
 * @returns {string}
 *
 * Note: the class "wp-element-button" is used to bring in the focus styling for the calendar date buttons,
 * But then the Tailwind classes are used to override all the non-focus styles, so the calendar remains compact.
 * This way we inherit the focus colouring of the main page without having to introduce new variables.
 *
 * Note: the <table> has relative positioning of right-2, and the day row (Mon, Tue etc) has left-2
 * This brings the table in line with the button text (not their padding [px-2])
 *
 */
...
DatePicker.prototype.setLanguage = function () {
	if (this.config.language) {
		...
		// with getDay() function
		const weekDays = this.dayLabels.map(item => item.toLowerCase());
		if (this.config.language === "Welsh") {
			this.dayLabelsAbbr = {
				Monday: ["Dydd ", "Llun", ""],
				Tuesday: ["Dydd ", "Maw", "rth"],
				Wednesday: ["Dydd ", "Mer", "cher"],
				Thursday: ["Dydd ", "Iau", ""],
				Friday: ["Dydd ", "Gwe", "ner"],
				Saturday: ["Dydd ", "Sad", "wrn"],
				Sunday: ["Dydd ", "Sul", ""],
			};
...
DatePicker.prototype.isExcludedDate = function (date) {
	...
	// see: https://github.com/ministryofjustice/wb-frontend/issues/923
...
DatePicker.prototype.updateCalendar = function () {
	this.$dialogTitle.innerHTML = `${this.monthLabels[this.currentDate.getMonth()]}<br />${this.currentDate.getFullYear()}`;
...
DatePicker.prototype.setCurrentDate = function (focus = true) {
	...
	this.calendarDays.forEach(calendarDay => {
		calendarDay.button.classList.add("wb-datepicker__button");
		calendarDay.button.classList.add("wb-datepicker__calendar-day");
...
DatePicker.prototype.isOpen = function () {
	return this.$dialog.classList.contains("wb-datepicker__dialog--open");
};
...
DatePicker.prototype.openDialog = function () {
	...
	this.$dialog.classList.add("wb-datepicker__dialog--open");
...
DatePicker.prototype.closeDialog = function () {
	...
	this.$dialog.classList.remove("wb-datepicker__dialog--open");
