            ...
            <div class="<?php echo $list_item_classes; ?> mb-4 flow-root">
                <?php if ($listing_settings["displayImage"]) {
                	...
                	if (!empty($thumb_url)) {
                		...
                		$thumb_class =
                			"wb-listing-thumbnail float-right w-[125px] h-[125px] md:w-[152px] md:h-[152px] bg-no-repeat bg-center ml-[5px] mb-[2px] border";
                ...
                <?php wb_blocks_filterable_listing_item_terms($listing_settings["displayTerms"]); ?>
                <?php wb_blocks_filterable_listing_item_details($display_fields); ?>
            </div>
            <?php
            } ?>
        </div>

    <?php wb_blocks_filterable_listing_pagination($listing_query);
	} else {
		 ?>
        <h2 class="font-bold text-2xl">
...
<div class="">
        <ul class="flex flex-wrap gap-2 p-0 m-0 list-none"> 
            <?php foreach ($taxTermsArray as $tax) {
