...
function wb_blocks_filterable_listing_block_get_listing_query($listing_settings, $active_filters)
{
	...
	if (!empty($listing_settings["restrictTaxonomies"]) && !empty($listing_settings["restrictTerms"])) {
		foreach ($listing_settings["restrictTerms"] as $restrict_term_id) {
			...
			if (
				!is_wp_error($restrict_term) &&
				$restrict_term &&
				in_array($restrict_term->taxonomy, $listing_settings["restrictTaxonomies"])
			) {
				$tax_qry_ary[] = [
	...
	if (!empty($published_date_qry)) {
		if (
			array_key_exists("after", $published_date_qry) &&
			array_key_exists("before", $published_date_qry) &&
			$published_date_qry["before"] < $published_date_qry["after"]
		) {
			// Swap dates if the end date is before the start date
			[$published_date_qry["after"], $published_date_qry["before"]] = [
				$published_date_qry["before"],
				$published_date_qry["after"],
			];
		}
	...
	if (!empty($meta_date_filters)) {
		foreach ($meta_date_filters as $meta_key => $meta_date_filter) {
			if (
				array_key_exists("fromDate", $meta_date_filter) &&
				array_key_exists("toDate", $meta_date_filter) &&
				$meta_date_filter["toDate"] < $meta_date_filter["fromDate"]
			) {
				// Swap dates if the end date is before the start date
				[$meta_date_filter["fromDate"], $meta_date_filter["toDate"]] = [
					$meta_date_filter["toDate"],
					$meta_date_filter["fromDate"],
				];
			}
...
function wb_blocks_filterable_listing_block_get_display_fields($display_fields)
{
	...
	foreach ($display_fields as $display_field) {
		...
		} elseif ($display_field == "published_date") {
