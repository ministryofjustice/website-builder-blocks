...
export const findProvider = embedCode => {
	return PROVIDERS.find(provider => provider.domains.some(domain => embedCode.includes(domain)));
};
...
export const extractDomains = embedCode => {
	...
	for (const match of embedCode.matchAll(stringPattern)) {
		...
		const domainMatch = stringValue.match(/^(?:https?:\/\/|\/\/)?((?:[a-z0-9-]+\.)+[a-z]{2,})(?=[:/?#]|$)/i);
...
export const validateAllowedDomains = (embedCode, provider) => {
	...
	return domains.every(domain =>
		provider.domains.some(approvedDomain => domain === approvedDomain || domain.endsWith(`.${approvedDomain}`)),
	);
};
...
export const validateEmbedCode = embedCode => {
   ...
   */
	...
	if (!validateAllowedDomains(embedCode, provider)) {
		return {
			isValid: false,
			message: "Embed code contains a domain that is not approved for this provider.",
			provider: provider,
		};
	}
