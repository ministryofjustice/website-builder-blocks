	...
	return (
		<>
			...
			<BlockControls>
				<ToolbarGroup>
					{mode === MODES.EDIT && (
						<Tooltip text={__("Click to validate the embed code.", "wb_blocks")}>
							<ToolbarButton
				...
				{mode === MODES.EDIT && (
					<>
						<TextareaControl
							...
							placeholder={__("Paste code from an approved provider.", "wb_blocks")}
						/>
						<p className="wb-allowed-third-party-embed__helper">
							{__("After pasting or editing the embed code, select Validate Code from the block toolbar.", "wb_blocks")}
						</p>

						{validationNotice && (
							<Notice status={validationNotice.status} isDismissible={false}>
								<strong>{__("Validation failed.", "wb_blocks")}</strong> {validationNotice.message}
							</Notice>
						)}
					</>
				)}
				...
				{mode === MODES.VALIDATED && (
					<Placeholder label={__("Third party Embed", "wb_blocks")}>
						<Notice status="success" isDismissible={false} className="wb-allowed-third-party-embed__notice">
							...
							<p>{__("Use the Edit button in the toolbar block to view or change the embed code.", "wb_blocks")}</p>
						</Notice>
					</Placeholder>
				)}
			</div>
