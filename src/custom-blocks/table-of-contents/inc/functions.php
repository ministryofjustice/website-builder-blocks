...
{
	if (!has_block("wb-blocks/table-of-contents", get_the_ID())) {
	...
	if (!empty($back_to_top_node)) {
		$back_to_top_text = esc_html($back_to_top_node->nodeValue);
	}
	...
	foreach ($tags as $tag) {
		...
		if (
			...
			continue;
		} // We ignore headings with the class "wb-toc-ignore" and the ToC's own H2
	...
	return ["index" => $index, "content" => $changed_content];
}
...
function wb_table_of_contents(
	...
	if (!$count_headings) {
		return "";
	} // No top headings for the table of contents = no table of contents
	...
	if ($count_headings > 15) {
		$print_columns = "wb-print-col wb-print-col--2";
	}
	if ($count_headings > 25) {
		$print_columns = "wb-print-col wb-print-col--3";
	}
