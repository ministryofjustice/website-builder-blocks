...
function wb_render_callback_accordion_block($attributes, $content)
{
	...
	$attribute_accordion_enable_open = $attributes['enableShowHideAll'] ?? true;
	...
	?>
	<?php if ($attribute_accordion_enable_open) {?>
...
function wb_render_callback_accordion_block_section($attributes, $content)
{
	...
	$attribute_accordion_uniqueID = $attributes['accordionUniqueName'] ?? "";
	...
	<details
		...
		<?php if (!empty($attribute_accordion_uniqueID)) echo "name=$attribute_accordion_uniqueID"; ?>
	>
