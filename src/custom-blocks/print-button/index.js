	...
	edit: props => {
		...
		return [
				...
				<PanelBody title={__("Print icon", "wb_block")} initialOpen={true}>
						...
						<ToggleControl
							...
							onChange={value => {
								setAttributes({
									...
									buttonText: value && buttonText.trim() === "" ? __("Print this page", "wb_block") : buttonText,
								});
							}}
						/>
					</PanelRow>
					...
					<PanelRow>
						<ToggleControl
							...
							help={
								!buttonShowText ? __("Edit icon size in the Styles panel", "wb_block") : __("Remove icon", "wb_block")
							}
					...
					{buttonShowIcon && buttonShowText && (
						<PanelRow>
							<RadioControl
								...
								help={__("Position icon left or right of the button text", "wb_block")}
					...
					{!buttonShowText && (
						<PanelRow>
							<TextControl
								...
								help={__("Add button action description for screen readers", "wb_block", "wb_block")}
								value={buttonText}
			...
			<InspectorControls group="styles">
				<PanelBody title={__("Print button styles", "wb_block")} initialOpen={true}>
					{!buttonShowText && buttonShowIcon ? (
						...
						<p>
							{__("Style options are currently available only when displaying an icon-only print button.", "wb_block")}
						</p>
					)}
				</PanelBody>
			</InspectorControls>,
			...
			<button
				className={`wb-print-button ${className || ""} wp-element-button ${buttonShowIcon ? "wb-print-button--has-icon" : ""} ${
